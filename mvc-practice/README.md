...
| 절차지향 | 객체지향 |
|----------|----------|
| 책임이 한곳에 집중 (예: getter 중심) | 책임이 여러 객체에 적절히 분산 |
| 데이터와 동작이 분리 | 데이터와 동작이 함께 존재 |

### 객체지향 설계 원칙
- **High Cohesion (높은 응집도)**: 관련 기능을 한 곳에 모음  
- **Loose Coupling (낮은 결합도)**: 변경 시 다른 부분에 미치는 영향 최소화  
- 설계는 서랍 정리와 유사 — 비슷한 성격은 모으고, 다른 성격은 분리

### 객체지향 설계 및 구현 순서 (6단계)
1. 도메인을 구성하는 객체 파악  
2. 객체 간 관계 설계  
3. 동적인 객체를 정적인 타입으로 추상화 및 도메인 모델링  
4. 협력 방식 설계  
5. 객체를 포괄하는 타입에 적절한 책임 할당  
6. 구현
